...
const Profile = () => {
    ...
    const user = {
        ...profileData,
        // Override with live context data if available and more recent
        ...
        <div className="profile-page-container">
            ...
            <main className="profile-content container">
                <div className="profile-grid">
                    ...
                    {/* Recent Activity Card - Now for all roles */}
                    <div className="profile-card activity-card" style={{ marginTop: '1.5rem', gridColumn: '1 / -1' }}>
                        <div className="card-header-icon bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400"><FileText size={20} /></div>
                        <h3>Recent Activity</h3>
                        <div className="activity-timeline-container" style={{ padding: '1rem' }}>
                            {(authUser?.recentActivity || user?.recentActivity) && (authUser?.recentActivity || user?.recentActivity).length > 0 ? (
                                <div className="sidebar-activity-list mt-0">
                                    {(authUser?.recentActivity || user?.recentActivity).map((activity) => (
                                        <div key={activity.id} className="sidebar-activity-item" style={{ marginBottom: '1rem', display: 'flex', gap: '12px' }}>
                                            <div className="sidebar-activity-dot mt-1.5" style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#4f46e5', flexShrink: 0 }}></div>
                                            <div className="sidebar-activity-content">
                                                <p className="sidebar-activity-text text-sm font-medium text-[var(--text-primary)]">{activity.text}</p>
                                                <span className="sidebar-activity-date text-xs text-[var(--text-muted)]">{activity.date}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <p className="empty-text text-[var(--text-muted)]">No recent activity found.</p>
                            )}
                        </div>
                    </div>
                </div>
