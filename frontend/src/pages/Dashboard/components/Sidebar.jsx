...
const Sidebar = ({ organization, activeTab, recentActivity = [] }) => {
    ...
    // Default letter 'N' for NGO if name is missing, ensuring consistency with display name
    const initials = (organization?.name || 'NGO').charAt(0).toUpperCase();
        ...
        <aside className="user-sidebar" style={{ width: '320px' }}>
            <div className="overflow-hidden" style={{ 
                ...
                <div className="profile-header flex items-center gap-5 p-6" style={{ borderBottom: '1px solid var(--border-color)' }}>
                    <div className="avatar-circle w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold text-white shrink-0 shadow-sm" style={{ backgroundColor: '#2563eb', color: '#ffffff' }}>
                        {initials}
                    </div>
