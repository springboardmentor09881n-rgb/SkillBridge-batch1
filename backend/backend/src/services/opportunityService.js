    ...
    const result = await pool.query(
     ...
     RETURNING id, title, description, "requiredSkills", location, duration, status, "ngoId", "createdAt"`,
        [
            oppData.title,
            oppData.description,
            JSON.stringify((oppData.requiredSkills || oppData.tags || []).map(s => s.toLowerCase().trim())),
            oppData.location,
            oppData.duration,
    ...
    const result = await pool.query(
     ...
     RETURNING *`,
        [
            oppData.title,
            oppData.description,
            oppData.requiredSkills ? JSON.stringify(oppData.requiredSkills.map(s => s.toLowerCase().trim())) : (oppData.tags ? JSON.stringify(oppData.tags.map(s => s.toLowerCase().trim())) : null),
            oppData.duration,
            oppData.location,
