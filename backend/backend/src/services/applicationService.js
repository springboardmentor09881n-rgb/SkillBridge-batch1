...
const applyForOpportunity = async (volunteerId, opportunityId, message) => {
    ...
    try {
        ...
        if (oppRes.rows.length > 0) {
            ...
            // 2. Official System Message (Confirmation)
            await sendSystemMessage(volunteerId, `Thanks for applying to "${title}"! Your profile has been shared with the NGO team. We'll notify you as soon as they review it. Good luck! ✨`);

            // 3. Real-time dashboard update for NGO (Ensure numeric ID room)
            const io = getIo();
            if (io) {
                io.to(Number(ngoId)).emit('statsUpdate', { type: 'new_application' });
                io.to(Number(volunteerId)).emit('statsUpdate', { type: 'application_submitted' });
            }
        }
    } catch (notifErr) {
...
const updateApplicationStatus = async (ngoId, applicationId, status) => {
    ...
    try {
        ...
        // 2. Official System Message (Acceptance/Rejection)
        if (status === 'accepted') {
            await sendSystemMessage(volunteerId, `Amazing news! You've been accepted for "${opportunityTitle}"! 🎉 The NGO is excited to have you. You can now start a conversation with them in the Messages tab!`);
        } else if (status === 'rejected') {
            await sendSystemMessage(volunteerId, `Thank you for your interest in "${opportunityTitle}". Although this one isn't a match, don't let it stop you! NGOs everywhere need your unique skills—keep exploring more opportunities and stay motivated! ✨`);
        } else if (status === 'pending') {
            await sendSystemMessage(volunteerId, `Good news! Your application for "${opportunityTitle}" is back under review. The NGO is taking another look—you still have a great chance! Stay tuned for updates. ✨`);
        }

        // 3. Real-time updates for both (Ensure numeric ID room)
        const io = getIo();
        if (io) {
            io.to(Number(volunteerId)).emit('statsUpdate', { type: 'status_changed', status });
            io.to(Number(ngoId)).emit('statsUpdate', { type: 'status_changed', status });
        }
    } catch (notifErr) {
        console.error('Notification error (non-fatal):', notifErr.message);
    }
